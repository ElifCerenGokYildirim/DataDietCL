...
class Forgetting(EarlyTrain):
    ...
    def after_loss(self, outputs, loss, targets, batch_inds, epoch):
        with torch.no_grad():
            ...
            batch_inds = torch.tensor(batch_inds).long()
    ...
    def before_run(self):
        ...
        self.forgetting_events = torch.zeros(self.n_train, requires_grad=False).to(self._device)
        self.last_acc = torch.zeros(self.n_train, requires_grad=False).to(self._device)
