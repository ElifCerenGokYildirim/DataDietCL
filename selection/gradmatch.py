...
class GradMatch(EarlyTrain):
    ...
    def calc_gradient(self, index=None, val=False):
        ...
        gradients = torch.zeros([sample_num, self.end_label * (self.embedding_dim + 1)],
        ...
        for i, (_, inputs, targets) in enumerate(batch_loader):
            ...
            with torch.no_grad():
                ...
                weight_parameters_grads = self._model.embedding_recorder.embedding["features"].cpu().view(batch_num, 1,
                                                    self.embedding_dim).repeat(1,self.end_label,1) *\
                                                    bias_parameters_grads.view(batch_num, self.end_label,
            ...
            if self.balance:
                ...
                for c in range(start_label, self.end_label):
                    ...
                    if self._device == "cpu":
                        ...
                        cur_weights = self.orthogonal_matching_pursuit(cur_gradients.to(self._device).T,
                                                                       cur_val_gradients.to(self._device),
                                                                       budget=round(len(class_index) * self.fraction))
