...
from .selection_utils import submodular_optimizer
import torch
import numpy as np
...
class Glister(EarlyTrain):
    def __init__(self, network, dst_train, args, fraction=0.5, random_seed=None, device=None, task_id = None, epochs=200, specific_model=None,
        ...
        self.eta = self.args["selection_lr"] if eta is None else eta
    ...
    def calc_gradient(self, end_label, index=None, val=False, record_val_detail=False):
        '''
        ...
        for i, (_, inputs, targets) in enumerate(batch_loader):
            ...
            with torch.no_grad():
                ...
                weight_parameters_grads = self._model.embedding_recorder.embedding["features"].view(batch_num, 1,
                                                self.embedding_dim).repeat(1, end_label, 1) *\
                                                bias_parameters_grads.view(
                                                batch_num, end_label, 1).repeat(1, 1, self.embedding_dim)
                ...
                if val and record_val_detail:
                    ...
                    self.init_emb.append(self._model.embedding_recorder.embedding["features"].cpu())
    ...
    def update_val_gradients(self, new_selection, selected_for_train):
        ...
        new_outputs = self.init_out - self.eta * sum_selected_train_gradients[:self.end_label].view(1,
                      -1).repeat(self.init_out.shape[0], 1) - self.eta * torch.matmul(self.init_emb,
                      sum_selected_train_gradients[self.end_label:].view(self.end_label, -1).T)

        sample_num = new_outputs.shape[0]
        gradients = torch.zeros([sample_num, self.end_label * (self.embedding_dim + 1)], requires_grad=False)
        i = 0
        while i * self.args.selection_batch < sample_num:
            ...
            weight_parameters_grads = self.init_emb[batch_indx].view(batch_num, 1, self.embedding_dim).repeat(1,
                                      self.end_label, 1) * bias_parameters_grads.view(batch_num,
                                      self.end_label, 1).repeat(1, 1, self.embedding_dim)
    ...
    def finish_run(self):
        ...
        self._model.embedding_recorder.record_embedding = True
        ...
        if self.balance:
            ...
            for c in range(start_label, self.end_label):
                ...
                c_val_inx = self.val_indx[self.dst_val.labels == c]
                self.calc_gradient(self.end_label,index=c_val_inx, val=True, record_val_detail=True)
                if self.dst_val != self.dst_train:
                    self.calc_gradient(self.end_label,index=c_indx)
