...
class DeepFool(EarlyTrain):
    ...
    def finish_run(self):
        ...
        if self.balance:
            ...
            for c in range(self.start_label,self.end_label):
    ...
    def deep_fool(self, inputs):
        ...
        self.start_label = self.task_id * 10
        ...
        cur_inputs = inputs.requires_grad_(True).to(self._device)
        ...
        first_preds = self.model(cur_inputs)["logits"].argmax(dim=1)
        self.model.no_grad = False

        for i in range(self.max_iter):
            f_all = self.model(cur_inputs)["logits"]

            w_k = []
            for c in range(self.start_label,self.end_label):
                w_k.append(torch.autograd.grad(f_all[:, c].sum(), cur_inputs,
                                               retain_graph=False if c + 1 == self.end_label else True)[
                               0].flatten(1))
            ...
            self.model.no_grad = True
            preds = self.model(cur_inputs)["logits"].argmax(dim=1)
            self.model.no_grad = False
